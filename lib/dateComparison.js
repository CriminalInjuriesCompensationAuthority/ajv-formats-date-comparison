...
function createValidatorService() {
    ...
    /**
     * Checks to see if the given parameter is not today's date.
     *
     * @param {string|Moment|Array|Date} input - A representation of a date
     * @return {Boolean} Boolean indicating if the input is not today's date. If it is not a valid date it will return false.
     *
     * @example
     *
     *     isNotToday('2020-06-25')
     */
    function isNotToday(input) {
        if (!isValidDateRepresentation(input)) {
            return false;
        }
        const inputtedDateTime = moment(input);
    ...
    /**
     * Compares 2 given dates. Compares the dates at the day level by default. i.e. '2020-06-24T10:30:15.000Z' and '2020-06-24T23:00:00.000Z'
     ...
     * @param {string} units - A unit of time to compare the dates by. default is `day` - Supported units: https://momentjs.com/docs/#/query/is-after/
     ...
     */
    function compare(a, b, units = 'day') {
        console.log({
            a,
            b,
            units
        ...
        if (
            ...
            // is a $.dollar reference to a date answer)
        ) {
            ...
            if (dateA.isAfter(dateB, units)) {
                return 1;
            }
            if (dateA.isBefore(dateB, units)) {
                return -1;
            }
            return 0;
        }
